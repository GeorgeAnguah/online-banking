...
class AuthRestApiTest {
    ...
    void setUp() {
        ...
        refreshUri = String.join("/", SecurityConstants.API_V1_AUTH_ROOT_URL, SecurityConstants.REFRESH_TOKEN);
    }
    ...
    void testLoginPathWithoutUsernameReturnsBadRequest(TestInfo testInfo) throws Exception {
        var loginRequest = TestUtils.toJson(new LoginRequest(StringUtils.EMPTY, testInfo.getDisplayName()));
    ...
    void testLoginPathWithoutPasswordReturnsBadRequest(TestInfo testInfo) throws Exception {
        var loginRequest = TestUtils.toJson(new LoginRequest(testInfo.getDisplayName(), StringUtils.EMPTY));
